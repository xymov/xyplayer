...
if(lsMobile()){	  
   ...
   $p2pinfo= $CONFIG["play"]['play']['all']['p2pinfo'];      
...
var api=is_mobile()? "<?php echo $CONFIG["parse"][$CONFIG["play"]["line"]["wap"]["line"]-1]['url'] ?>":"<?php echo $CONFIG["parse"][$CONFIG["play"]["line"]["pc"]["line"]-1]['url']  ?>" ;

var urljmp='<?php echo $CONFIG["play"]["match"]["urljmp"]?>';

var autoplay="<?php echo $autoplay ?>";

var p2pinfo="<?php echo $p2pinfo ?>";
...
function player(){ $.ajax({ url: 'api.php?cip='+cip+'&url=' + url,dataType: 'jsonp',jsonp: 'cb',success:function(data){success(data);}}); };
 ...
 function success(data){
  ...
  if (data.success){
		...
		if(data['url'].search(/\.(ogg|mp4|webm|m3u8)$/i)!==-1){			
				...
				urlplay(myplayer+encodeURIComponent(data.url)+"&autoplay="+autoplay+"&p2pinfo="+p2pinfo);				
 ...
 // function getcip(){$.get("https://data.video.iqiyi.com/v.f4v",function(cdnip){sip=cdnip.match(/\d+\.\d+\.\d+\.\d+/);cip=sip[0];player();},"text");}
 
 //
 
 function testurl(jmp,word){
 ...
 if(jmp!=="" && word!=="")
{
 ...
 var jmp=testurl(urljmp,url);
 
 if(jmp!==false){
     
    urlplay(jmp);
     
 }else if(url.search(/\.(ogg|mp4|webm|m3u8)$/i)!==-1){
     
     urlplay(myplayer+encodeURIComponent(url)+"&autoplay="+autoplay);
